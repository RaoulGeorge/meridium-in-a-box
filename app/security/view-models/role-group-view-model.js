define(function (require) {
    ...
    DialogScreen = require('./role-group-popover-view-model'),
    view = require('text!../views/role-group.html');

    require('ui/elements/list-group/view-model');


    function RoleGroupViewModel(kom, applicationEvents, securityService) {
        base.call(this, view);
        var self = this;
        this.kom = kom;
    ...
    var base = Object.inherit(KnockoutViewModel, RoleGroupViewModel);
    RoleGroupViewModel.dependsOn = [KnockoutManager, ApplicationEvents, SecurityService];

    ///////////////////
    // Lifecycle
    ///////////////////

    RoleGroupViewModel.prototype.load =
        function roleGroupViewModel_load(routeArgs) {
            var self = this,
                dfd = new $.Deferred();
    ...
    RoleGroupViewModel.prototype.activate =
        function roleGroupViewModel_activate() {
    ...
    RoleGroupViewModel.prototype.attach =
        function roleGroupViewModel_attach(region,selectedRole) {
            ...
            this.assignedList.loader = loadGroups.bind(null, this, selectedRole.key());
            this.selectedRole = selectedRole;
            this.selected([]);
        };

    RoleGroupViewModel.prototype.detach =
        function roleGroupViewModel_detach(region) {
            base.prototype.detach.call(this, region);
        };

    RoleGroupViewModel.prototype.canUnload =
        function roleGroupViewModel_canUnload() {
            return !this.isDirty();
        };

    RoleGroupViewModel.prototype.deactivate =
        function roleGroupViewModel_deactivate() {
            this.kom.disposeSubscriptions();
            this.kom.disposeComputeds();
        };

    RoleGroupViewModel.prototype.unload =
        function roleGroupViewModel_unload() {
    ...
    RoleGroupViewModel.prototype.onAdd =
       function roleGroupViewModel_onGroupAdd(data, event) {
           var vm = ko.contextFor(event.target).$root;
           vm.show();
       };

    RoleGroupViewModel.prototype.clearDirty =
   function roleGroupViewModel_clearDirty() {
       this.kom.tracker.markCurrentStateAsClean();
   };

    RoleGroupViewModel.prototype.selectAll =
      function roleGroupViewModel_selectAll(data, event) {
    ...
    RoleGroupViewModel.prototype.onSelected =
       function roleGroupViewModel_onGroupSelected(data, event) {
    ...
    RoleGroupViewModel.prototype.save =
        function roleGroupViewModel_save() {
            var groupsToAdd, groupsToDelete;
            var self = this;
            // If no changes, nothing to do.
            if (this.assignedNewChangeLog.length > 0 || this.assignedRemoveChangeLog.length > 0) {
                this.isLoading(true);
                if (this.assignedNewChangeLog.length > 0) {
                    groupsToAdd = createRoleDTOAdd(this);
                    this.securityService.addRoleGroups(groupsToAdd)
                        .done(addRoleGroups_done.bind(null, this))
                        .fail(handleAjaxRequestError.bind(null, this));
                }
                if (this.assignedRemoveChangeLog.length > 0) {
                    groupsToDelete = createRoleDTODelete(this);
                    groupsToDelete.groups.forEach(function (item) {
                        self.securityService.deleteRoleGroups(self.selectedRole.key(), item.key)
                        .done(removeRoleGroups_done.bind(null, self))
    ...
    RoleGroupViewModel.prototype.delete =
        function roleGroupViewModel_delete(data, event) {
            var vm = ko.contextFor(event.target).$root;
            promptDeleteGroups(vm, confirmDeleteGroups_done.bind(null, vm));
        };

    RoleGroupViewModel.prototype.translate = function (key) {
        return this.translator.translate(key);
    };

    RoleGroupViewModel.prototype.clearLists =
    function roleGroupViewModel_clearList(data, event) {
        this.assignedNewChangeLog = [];
        this.assignedRemoveChangeLog = [];
    };

    RoleGroupViewModel.prototype.hasChanges = function hasChanges() {
        return this.isDirty();
    };
    ...
    function loadGroups(self, roleKey, page, pageSize) {
        var dfd = $.Deferred();

        if (roleKey!=='0') {
            var groups = self.securityService.getRoleGroups(roleKey)
                .done(getRoleGroups_done.bind(null, self, dfd))
    ...
    function getRoleGroups_done(self, dfd, dtos) {
        self.assigned(sortByCaption(dtos));
    ...
    function loadAllDone(self) {
        ...
        self.assignedList.items = sortByCaption(temp);
    ...
    function removeRoleGroups_done(self) {
        clearIsDirty(self);
        self.isLoading(false);
        self.assignedList.items = sortByCaption(self.assignedList.items);
        self.assigned(self.assignedList.items);
        self.assignedRemoveChangeLog = [];
    }

    function addRoleGroups_done(self) {
        clearIsDirty(self);
        self.isLoading(false);
        self.assignedList.items = sortByCaption(self.assignedList.items);
    ...
    function promptDeleteGroups(self, doneCallback) {
        var msg = self.translate('SEC_ROLEGROUP_CONFIRM_DELETE_CURRENT'),
    ...
    function createHash(self) {
        ...
        hashObject = {
            assignedGroups: _.map(self.assigned(), function (group) {
                return group.key;
            })
        };
