...
define(function (require) {
    ...
    var ko = require('knockout'),
        ...
        view = require('text!../views/user-detail-sites-dialog.html');

    require('system/lang/object');
    require('ui/elements/searchbox/view-model');


    function UserDetailSitesDialogViewModel(securityService) {
        base.call(this, view);
    ...
    var base = Object.inherit(KnockoutViewModel, UserDetailSitesDialogViewModel);

    UserDetailSitesDialogViewModel.dependsOn = [SecurityService];

    UserDetailSitesDialogViewModel.prototype.activate = function () {
        // Computed observables.
        this.kom.subscribe(this.filteredGroups, filteredGroups_subscribe.bind(null, this));
    };

    UserDetailSitesDialogViewModel.prototype.load = function roleGroupPopoverViewModel_load() {
        ...
        this.service.getSites()
            .done(getSites_done.bind(null, this, dfd))
            .fail(handleAjaxRequestError.bind(null, this, dfd));

        return dfd.promise();
    };

    UserDetailSitesDialogViewModel.prototype.attach = function (region) {
    ...
    UserDetailSitesDialogViewModel.prototype.detach = function RoleGroupPopoverViewModel_detach(region) {
        base.prototype.detach.call(this, region);
    };

    UserDetailSitesDialogViewModel.prototype.unload = function RoleGroupPopoverViewModel_unload() {
        this.kom.disposeSubscriptions();
        this.kom.disposeComputeds();
        this.kom.disposeObservables();
    };

    UserDetailSitesDialogViewModel.prototype.loadGroup = function loadGroup() {
        var dfd = $.Deferred();
        dfd.resolve(this.filteredGroups());
        return dfd.promise();
    };

    UserDetailSitesDialogViewModel.prototype.deactivate = function () {
        this.kom.dispose();
    };



    UserDetailSitesDialogViewModel.prototype.save = function () {
        return this.availableGroups.selectedItems;
    };

    UserDetailSitesDialogViewModel.prototype.translate = function (key) {
    ...
    function filteredGroups_read(self) {
        ...
        if (!self.groupFilter()) {
            ...
            values = ko.utils.arrayFilter(values, function (u) {
                return u.name.toLowerCase().startsWith(filter);
            });
            retValues = values;
        }
        return retValues;
    ...
    function sortGroups(groups) {
        return _.sortBy(groups, sortByCaseInsensitive.bind(null, 'name'));
    }
    ...
    return UserDetailSitesDialogViewModel;
});