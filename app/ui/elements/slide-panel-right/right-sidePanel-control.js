define(function (require) {
    ...
    slidePanelRightProto.positionSelf = function () {
        ...
        $(this).find('div.slide-tab').css('top', (predecessorCount * TONGUE_HEIGHT + 10));
    };

    slidePanelRightProto.getPredecessorCount = function () {
        var allSlidePanelRight = $('mi-slidepanelright');
    ...
    slidePanelRightProto.openHalf = function () {
        ...
        $('mi-slidepanelright').not(this).hide();
    ...
    function checkIfPanelIsDisabled(self) {
        self.isPanelDisabled = self.hasAttribute("disabled");
        if (self.isPanelDisabled && self.getAttribute("disabled") && self.getAttribute("disabled").toLowerCase() === "false") { //to support disabled='false'
            self.isPanelDisabled = false;
        }
    }

    function setDisabledPanel(self) { //apply disabled css
        if (self.isPanelDisabled) {
            $(self).addClass("slidepanel-disabled");
        }
        else {
            $(self).removeClass("slidepanel-disabled");
        }
    }

    var slidePanelRight = document.registerElement("mi-slidepanelright", { prototype: slidePanelRightProto });
